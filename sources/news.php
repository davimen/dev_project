<?php  if(!defined('_source')) die("Error");
		...
		if($id!=''){
			...
			$sql = "select * from #_baiviet_photo where type='".$type_bar."' and  id_baiviet='".$row_detail['id']."'";
			$d->query($sql);
			$result_arrayPhoto = $d->result_array();

			
			
			
			#các tin cu hon
			  $sql_khac = "select ten_$lang,ngaytao,id,tenkhongdau,thumb,photo,mota_$lang from #_baiviet where hienthi=1 and id !='".$row_detail['id']."' and type='".$type_bar."' order by stt,ngaytao desc limit 0,10";
			$d->query($sql_khac);
