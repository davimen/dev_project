    ...
    <ul>
        ...
        <?php for($j=0,$count_sp=count($product);$j<$count_sp;$j++){?>
             <li class="thuonghieu_item">
                    <a href="<?=$product[$j]['link']?>" target="_blank"><img src="<?=_upload_hinhanh_l.$product[$j]['thumb_vi']?>" alt="<?=$product[$j]['ten_'.$lang]?>" /></a>
                    <h3><a href="<?=$product[$j]['link']?>" target="_blank"><?=$product[$j]['ten_'.$lang]?></a></h3>
                </li> 
        <?php } ?>
		<?php } else { ?>
